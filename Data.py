...
while True:
    print('1 - Count names')
    print('2 - Count emails')
    print('3 - Count names of files')
    print('4 - Count colors')
    print('5 - Exit')
    ...
    if command == '5':
        ...
        with open('name_surname.txt', 'w') as file:
            ...
            for name in names_list: file.write(name+'\n')
    elif command == '2':
        with open('emails.txt', 'w') as file:
            emails_list = re.findall(r'(\b[\w\-]+[@][\w\-]+(\.[\w\-]+)+)', content)
            for email in emails_list: file.write(email[0]+'\n')
    elif command == '3':
        with open('file.txt', 'w') as file:
            files_list = re.findall(r'[\t\s][\w]+\.[\w]+\b', content)
            for files in files_list: file.write(files[1:]+'\n')
    elif command == '4':
        with open('colors.txt', "w") as file:
            color_list = re.findall(r'#[a-f0-9]{6}\n', content)
            for color in color_list: file.write(color)
    else:
